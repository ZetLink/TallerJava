...
public class Lista {
    ...
    //Insertar
    public void insertarPrim(Vuelos e){
        Nodo N = new Nodo(e);
        if (list != null){
            N.setPs(list);
        }
    ...
    public void insertar(Vuelos entrada) {
        Nodo p = inicio();
        Nodo ant = null;
        int b = 0;
        
        while (p != null && b == 0) {
            if (p.getDato().getCod_vuelo() >= entrada.getCod_vuelo()) {
                b = 1;
            } else {
                ant = p;
                p = p.getPs();
    ...
    public Nodo eliminarElemento(int entrada){
        ...
        boolean band = false;
        while(p!= null & !band){
            Vuelos vuelo = p.getDato();
            if (vuelo.getCod_vuelo() >= entrada) {
                if (vuelo.getCod_vuelo()== entrada) {
                    band = true;
                }else{
                    p = null;
